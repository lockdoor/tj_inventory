...
class TestPettyCashAccountViews:
    ...
    def test_unauthenticated_redirect(self, client):
        response = client.get(reverse('accounting:account-list'))
    ...
    def test_list_view_and_search(self, client, manager_user, account):
        ...
        # Search match
        response = client.get(reverse('accounting:account-list'), {'q': 'HO-01'})
        assert len(response.context['accounts']) == 1

        # Search mismatch
        response = client.get(reverse('accounting:account-list'), {'q': 'NonExistent'})
        assert len(response.context['accounts']) == 0

    def test_detail_view(self, client, manager_user, account):
        client.force_login(manager_user)
        response = client.get(reverse('accounting:account-detail', kwargs={'pk': account.pk}))
        assert response.status_code == 200
        assert response.context['account'] == account

    def test_create_view_post_success(self, client, manager_user, company, custodian_user):
        client.force_login(manager_user)
        url = reverse('accounting:account-create')
        data = {
            'code': 'PC-HQ-02',
            'name': 'HQ Sub-Box',
            'company': company.pk,
    ...
    def test_update_view_fields_locked(self, client, manager_user, account, custodian_user):
        client.force_login(manager_user)
        url = reverse('accounting:account-update', kwargs={'pk': account.pk})
    ...
    def test_delete_trash_restore_lifecycle(self, client, manager_user, account):
        ...
        # 1. Soft delete
        delete_url = reverse('accounting:account-delete', kwargs={'pk': account.pk})
        ...
        # 2. View Trash
        trash_url = reverse('accounting:account-trash')
        response = client.get(trash_url)
        assert response.status_code == 200
        assert account in response.context['accounts']

        # 3. Restore
        restore_url = reverse('accounting:account-restore', kwargs={'pk': account.pk})
