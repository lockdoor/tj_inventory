...
def test_dashboard_access_for_logged_in_user(client, test_user):
    ...
    assert "Company Directory" in content
    assert "Individual Registry" in content
    ...
    assert reverse('common:company-list') in content
    assert reverse('common:individual-list') in content
