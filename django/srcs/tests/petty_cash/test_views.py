...
class TestPettyCashViews:
    ...
    def test_category_detail_view(self, client, user_with_perms, category):
        client.force_login(user_with_perms)
        response = client.get(reverse('petty_cash:category-detail', kwargs={'pk': category.pk}))
        assert response.status_code == 200
    ...
    def test_category_create_view(self, client, user_with_perms, company):
        ...
        # GET create form
        response = client.get(reverse('petty_cash:category-create'))
        assert response.status_code == 200
        ...
        response = client.post(reverse('petty_cash:category-create'), data)
    ...
    def test_category_update_view(self, client, user_with_perms, category):
        ...
        # GET update form
        response = client.get(reverse('petty_cash:category-update', kwargs={'pk': category.pk}))
        assert response.status_code == 200
        ...
        response = client.post(reverse('petty_cash:category-update', kwargs={'pk': category.pk}), data)
        assert response.status_code == 302
    ...
    def test_category_delete_trash_restore_lifecycle(self, client, user_with_perms, category):
        ...
        response = client.post(reverse('petty_cash:category-delete', kwargs={'pk': category.pk}))
        assert response.status_code == 302
        ...
        response = client.get(reverse('petty_cash:category-trash'))
        assert response.status_code == 200
        assert category in response.context['categories']

        # 3. Restore category
        response = client.post(reverse('petty_cash:category-restore', kwargs={'pk': category.pk}))
        assert response.status_code == 302
