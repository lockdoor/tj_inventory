...
from petty_cash.models import PettyCashAccount, PettyCashCategory, PettyCashPayment, PettyCashPaymentItem
...
class TestPettyCashPaymentViews:
    ...
    def test_list_view(self, client, manager_user, account, payment):
        ...
        url = reverse('petty_cash:payment-list', kwargs={'account_code': account.code})
    ...
    def test_create_payment_success(self, client, manager_user, account, category):
        ...
        url = reverse('petty_cash:payment-create', kwargs={'account_code': account.code})
        data = {
            'payment_type': 'disbursement',
    ...
    def test_update_payment_recalculates_balance(self, client, manager_user, account, category, payment):
        ...
        url = reverse('petty_cash:payment-update', kwargs={'pk': payment.pk})
    ...
    def test_cancel_payment_reverses_balance(self, client, manager_user, account, payment):
        ...
        url = reverse('petty_cash:payment-cancel', kwargs={'pk': payment.pk})
    ...
    def test_posted_payment_locking_controls(self, client, manager_user, payment, category):
        ...
        # 1. Try to update
        update_url = reverse('petty_cash:payment-update', kwargs={'pk': payment.pk})
        data = {
            'payment_type': 'disbursement',
            'payment_date': '2026-07-03',
            'items-TOTAL_FORMS': '1',
        ...
        # 2. Try to cancel
        cancel_url = reverse('petty_cash:payment-cancel', kwargs={'pk': payment.pk})
    ...
    def test_summary_view_and_eom_posting(self, client, manager_user, account, payment):
        ...
        url = reverse('petty_cash:payment-summary', kwargs={'account_code': account.code})
        response = client.get(url)
        assert response.status_code == 200
